...
namespace Engine
{
	...
	hashindex ResourceAllocator::LoadResource(const char* a_FilePath, Resource::ResourceType a_Type)
	{
		...
		m_Resources.Insert(t_Hash, resource);
	...
	{
		Logger::Assert(m_Resources.Find(GetHashFromPath(a_FilePath))->Unload(), "Failed to unload Resource");
	}

	void ResourceAllocator::UnloadResource(hashindex a_ID)
	{
		Logger::Assert(m_Resources.Find(a_ID)->Unload(), "Failed to unload Resource");
	}
